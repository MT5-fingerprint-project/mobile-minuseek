import { OidcTokens, refreshTokens, revokeRefreshToken, signInWithSlug } from './oidc'
import { clearSession, loadSession, saveSession, StoredSession } from './session-store'
...
function toStoredSession(slug: string, tokens: OidcTokens, previousRefreshToken: string | null): StoredSession {
  return {
    slug,
...
export async function signOut(): Promise<void> {
  ...
  if (endingSession?.refreshToken) {
    try {
      await revokeRefreshToken(endingSession.slug, endingSession.refreshToken)
    } catch {
...
export async function getValidAccessToken(): Promise<string | null> {
  ...
  try {
    const tokens = await refreshTokens(activeSession.slug, activeSession.refreshToken)
    activeSession = toStoredSession(activeSession.slug, tokens, activeSession.refreshToken)
    await saveSession(activeSession)
    return activeSession.accessToken
  } catch {
    await signOut()
